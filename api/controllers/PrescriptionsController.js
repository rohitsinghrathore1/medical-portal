...
module.exports = {
 ...
 getPrescriptionDetails: function(req, res, next) {
    ...
    Prescriptions.findOne({id: prescriptionId}).exec(function(err, prescription) {
      if (err) {
        return res.serverError(err);
      }

      if(userId === prescription.user_id) {
        return res.json({
          prescription,
          status: "SUCCESS"
        });
        ...
        UserPrescription.findOne({prescription_id: prescriptionId, user_id: userId}).exec(function(err, userPrescription) {
          if (err) {
            return res.serverError(err);
          }
          console.log(" userPrescription = ", userPrescription);
          if(userPrescription && userPrescription.approval_status === "accessAllowed"){
            return res.json({
              prescription,
              status: "SUCCESS"
            });
